...
* 连接认证
  ...
  它使用了TLS这样的加密模型，但并不是整个TLS。它只使用了TLS模型中用于加密的最小的一部分。（We do not use TLS directly, because we do not want the CA system baggage. Most TLS implementations are very big. Since the libp2p model begins with keys, libp2p only needs to apply ciphers. This is a minimal portion of the whole TLS standard.）
